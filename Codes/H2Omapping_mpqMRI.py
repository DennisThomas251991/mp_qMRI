...
from Useful_functions import Bias_field_corr_SPM
...
class H2O_map_mpqMRI():
    ...
    def T1_correction(self):
        ...
        M0_T1corr_path = os.path.join(self.dst1, 'M0_T1corr.nii')
    ...
    def Calc_CSF_calibration_factor(self):
        ...
        if self.arguments['Bias_field_corr_method'] == 'SPM':
            masked_brain_path2 = os.path.join(self.dst2, self.filename2 + '.nii.gz')
            masked_brain_extracted = os.path.join(self.dst2, self.filename2 + '.nii')
            with gzip.open(masked_brain_path2, 'rb') as f_in:
                with open(masked_brain_extracted, 'wb') as f_out:
                    shutil.copyfileobj(f_in, f_out)
            Bias_field_corr_SPM(masked_brain_extracted)
            CSF_mask = nib.load(os.path.join(self.dst2, 'c3' + self.filename2 + '.nii')).get_fdata()
            GM_mask = nib.load(os.path.join(self.dst2, 'c2' + self.filename2 + '.nii')).get_fdata()
            WM_mask = nib.load(os.path.join(self.dst2, 'c1' + self.filename2 + '.nii')).get_fdata()
        else:
            masked_brain_path2 = os.path.join(self.dst2, self.filename2 + '_brain.nii.gz')
            fsl_fastseg(masked_brain_path2)
            CSF_mask = nib.load(os.path.join(self.dst2, self.filename2 + '_brain_pve_0.nii.gz')).get_fdata()
            GM_mask = nib.load(os.path.join(self.dst2, self.filename2 + '_brain_pve_1.nii.gz')).get_fdata()
            WM_mask = nib.load(os.path.join(self.dst2, self.filename2 + '_brain_pve_2.nii.gz')).get_fdata()
    ...
    def bias_field(self, path):
        if self.arguments['Bias_field_corr_method'] == 'SPM':
            ...
            path_brain_mask = os.path.join(self.dst2, self.filename2 + '_brain_mask.nii.gz')
            ...
            path_mod = os.path.join(self.dst2, 'mod_M0_T1corr.nii')
            nib.save(nii, path_mod)

            Bias_field_corr_SPM(path_mod)
            ...
            bias_field_path = os.path.join(dst, 'BiasField_mod_M0_T1corr.nii')
        else:
            fsl_fastseg(path)
            ...
            bias_field_path = os.path.join(dst, filename + '_bias.nii.gz')
    ...
    def masking(self):
        ...
        masked_brain_path1 = os.path.join(self.dst1, self.filename2 + '_brain.nii.gz')
        brain_mask = nib.load(os.path.join(self.dst1, self.filename2 + '_brain_mask.nii.gz')).get_fdata()
