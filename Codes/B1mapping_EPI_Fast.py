...
def B1mapping_EPI_fast(path, name_b0map, name_EPI_45deg, name_EPI_90deg,sign_gradient):
    ...
    name_b1map = 'B1_MAP_from_fast_EPI_standard.nii'

    # Non-Extended data
    # B1 map with B0 correction:
    name_b1map_nonext = 'B1_MAP_from_fast_EPI_standard_nonext.nii'
    ...
    nii_b1map = nib.Nifti1Image(b1map_improved_extended, None, header=info)
    nib.save(nii_b1map, name_b1map)

    nii_b1map_nonext = nib.Nifti1Image(b1map_improved, None, header=info)
