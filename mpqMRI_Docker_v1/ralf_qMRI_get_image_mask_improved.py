...
# This function works for data where the bulk part of the signal is concentrated in an inner volume with noise outside
  ...
  # Empty object for data from corners:
    ...
    corners[:, :, :, 0] = vol[0:sz1, 0:sz2, 0:sz3]
    corners[:, :, :, 1] = vol[0:sz1, -sz2:, 0:sz3]
    corners[:, :, :, 2] = vol[-sz1:, 0:sz2, 0:sz3]
    corners[:, :, :, 3] = vol[-sz1:, -sz2:, 0:sz3]
    corners[:, :, :, 4] = vol[0:sz1, 0:sz2, -sz3:]
    corners[:, :, :, 5] = vol[0:sz1, -sz2:, -sz3:]
    corners[:, :, :, 6] = vol[-sz1:, 0:sz2, -sz3:]
    ...
    for i in range(8):
        ...
        myhelp = corners[:, :, :, i]
        myhelp_reshaped = myhelp.reshape(-1, order='F')

        # Find indices where myhelp is greater than 0
        indices = np.where(myhelp_reshaped > 0)[0]
        
        # Extract values based on indices
        data = myhelp_reshaped[indices]
    ...
    spx = np.round(np.sum(vol0 * xx / mass),4)
    spy = np.round(np.sum(vol0 * yy / mass),4)
    ...
    Iz = np.round(np.sum(vol0 * (xx**2 + yy**2)),4)
    ...
    if max_holesize>0:
        ...
        for i in range(maxsize, 2, -2):
            ...
            ksum = np.sum(kernel)
            myhelp = np.round(ralf_qMRI_conv_3D.ralf_qMRI_conv_3D(mymm, kernel))
