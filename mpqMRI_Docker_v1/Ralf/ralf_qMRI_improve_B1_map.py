...
import ralf_qMRI_calc_local_mean_and_std
import ralf_qMRI_fill_holes_in_mask_3D_v02
import ralf_qMRI_get_edges_of_mask
import ralf_qMRI_conv_3D_fast
def ralf_qMRI_improve_B1_map(b1map,minb1,maxb1,nlayers):
    ...
    for i in range(1, 3):
        ...
        numpix = np.round(ralf_qMRI_conv_3D((b1map > 0), kernel))
    ...
    kernel = np.ones(3, 1)
    non_zero_neighbors = np.round(ralf_qMRI_conv_3D((b1map > 0), kernel))
    b1map[(b1map > 0) & (non_zero_neighbors < 5)] = 0

    # STEP 4: Smooth B1 map
    kernel = np.ones(3, 1)
    b1_integral = np.abs(ralf_qMRI_conv_3D_fast(b1map, kernel))
    numpix = np.round(abs(ralf_qMRI_conv_3D_fast((b1map > 0), kernel)))
    b1map_sm = b1_integral / (np.finfo(float).eps + numpix) * (numpix > 0) * (b1map > 0)
    ...
    for _ in range(nlayers):
        edge1, edge2 = ralf_qMRI_get_edges_of_mask((b1map_curr>0));
        b1map_fitted, _ = ralf_qMRI_calc_local_mean_and_std(b1map_curr, (b1map_curr > 0), 5)
        b1map_ext = b1map_curr + b1map_fitted * (b1map_curr == 0)
        b1map_ext = b1map_ext * (b1map_ext > minb1) * (b1map_ext < maxb1)
        b1map_curr = b1map_ext.copy()

    # b1map_improved_extended is now b1map_sm wherever this is non-zero and b1map_ext elsewhere:
    b1map_improved_extended = b1map_sm * (b1map_sm > 0) + b1map_ext * (b1map_sm == 0)

    # For export of non-extended B1:
    maske_fill = ralf_qMRI_fill_holes_in_mask_3D_v02(b1map_sm > 0, 9)
