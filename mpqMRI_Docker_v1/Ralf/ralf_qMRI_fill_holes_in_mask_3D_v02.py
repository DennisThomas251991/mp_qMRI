...
def ralf_qMRI_fill_holes_in_mask_3D_v02(vol, minnb):
    ...
    g1l, g1u = np.min(np.where(np.sum(np.sum(vol, axis=2), axis=1) > 0)), np.max(np.where(np.sum(np.sum(vol, axis=2), axis=1) > 0))
    g2l, g2u = np.min(np.where(np.sum(np.sum(vol, axis=0), axis=1) > 0)), np.max(np.where(np.sum(np.sum(vol, axis=0), axis=1) > 0))
    g3l, g3u = np.min(np.where(np.sum(np.sum(vol, axis=0), axis=0) > 0)), np.max(np.where(np.sum(np.sum(vol, axis=0), axis=0) > 0))
